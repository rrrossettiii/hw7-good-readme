...
function generateMarkdown(userResponses, userGitHub) {
  ...
  \n![Badge for GitHub last commit](https://img.shields.io/github/last-commit/${userResponses.Username}/${userResponses.Repository}?style=flat&logo=appveyor)![Badge for Top Language](https://img.shields.io/github/languages/top/${userResponses.Username}/${userResponses.Repository}?style=flat&logo=appveyor)
  \n&nbsp;\n \n&nbsp;\n
  \n## Description:
  ...
  \n&nbsp;\n \n&nbsp;\n`;
	...
	if (
		...
		instructionsArray = userResponses.Installation.replace(
			/([.?!])\s*(?=[A-Z])/g,
			"$1|"
		).split("|");
		numberedInstructionsArray = instructionsArray.map(
			(arr, index) => `${index + 1}.\t` + arr
		);
